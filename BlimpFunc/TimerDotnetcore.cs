        ...
        public static async System.Threading.Tasks.Task RunAsync([TimerTrigger("0 8 * * *")]TimerInfo myTimer, ILogger log)
        {
            ...
            try
            {
                ...
                List<String> newTags = await dockerhubUtils.PollDockerhubRepos("https://registry.hub.docker.com/v2/repositories/oryxprod",
                    new Regex("^dotnetcore-[0-9]+\\.[0-9]+$", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                    new Regex("^(?!.*latest).*$", RegexOptions.Compiled | RegexOptions.IgnoreCase),
                    DateTime.Now.AddDays(-1));

                log.LogInformation(String.Format("Dotnetcore: {0} tags found {1}", newTags.Count, String.Join(", ", newTags)));
                telemetry.TrackEvent(String.Format("Dotnetcore: {0} tags found {1}", newTags.Count, String.Join(", ", newTags)));
                foreach (String t in newTags)
                {
                    try
                    {
                        ...
                        String url = String.Format("https://Blimpfunc.azurewebsites.net/api/HttpDotnetcorePipeline?code={0}", secretsUtils._BlimpfuncMaster);
